...
$export_dir = Yii::getPathOfAlias('webroot').'/images/uploads/'; # final slash is required
// END SETTINGS
...
//if ($message_count < 1) {
	for ($msgid = 1; $msgid <= $message_count; ++$msgid) {
		...
		if (isset($overview[0]->subject)){ $subject = $overview[0]->subject; echo 'Subject : '.$subject.'</br>';}
		echo 'Message : '.$message.'</br>';
		//print_r($attach);

		$latitude = getLatitude($message);
		$longitude = getLongitude($message);
		$sql = "INSERT INTO `tbl_maps` (`time`, `email`, `subject`, `detail`, `latitude`, `longitude`) VALUES ('".$prettydate."', '".$email."', '".$subject."', '".$message."', '".$latitude."', '".$longitude."')";
		$inMaps = Yii::app()->db->createCommand($sql)->execute();
		echo $sql;
		
		if ($inMaps && $attach) {
			$lastId = Yii::app()->db->createCommand('SELECT id FROM `tbl_maps` ORDER BY id DESC LIMIT 1')->queryScalar();
			foreach ($attach as $data) {
				$sqlImage = "INSERT INTO `tbl_images` (`path`, `maps_id`) VALUES ('".$data."', '".$lastId."')";
				$inImages = Yii::app()->db->createCommand($sqlImage)->execute();
